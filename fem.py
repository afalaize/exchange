# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 18:51:33 2016
...
def u_init(pars_):
	return dolfin.Expression('A*exp(pow(x[0]-0.5, 2) + pow(x[1]-0.5, 2))', A=dolfin.Constant(pars_['Amp_init']))

# init boundary conditions on function spaces
def BC_u(FS_, pars_):
    # Base Boundary Conditions
    bc_sud = dolfin.DirichletBC(FS_, pars_['T_sud'],
...
def mesh(pars_):
    # Base mesh
    return dolfin.UnitSquareMesh(pars_['Nx'],
                                 pars_['Ny'])
...
class Functions(object):
	...
	def __init__(self, fem, mesh)
        ...
        for name in self.vars_.keys():
        	...
        	setattr(self, name+'_init', init(fem.pars))
	        ...
	        def builder(funcspace_, elements_, name_, tempBC_):	        	
		        ...
		        def BC():
		        	return tempBC_(getattr(self, name_+'_FS')(), fem.pars)
...
class Fem(object):
    # vars_dic = {'varname': type} with type = 'scalar' or 'vector'
